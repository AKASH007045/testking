                ...
                if (b.info) {
                    ...
                    if (!this._.selectedElement)
                        for (var h = c.append(l("tbody")), e = parseInt(b.txtRows, 10) || 0, f = parseInt(b.txtCols, 10) || 0, i = 0; i < e; i++) {
							...
							/**Patch for Cellpadding in td**/
                            for (var j = h.append(l("tr")), k = 0; k < f; k++) {
								if (c.getAttribute('cellpadding') != null || c.getAttribute('cellpadding')!= ""){
									j.append(l("td").setStyle('padding', c.getAttribute('cellpadding')+'px')).appendBogus();
								}
								else{
                    ...
                    elements: [{
                        ...
                        children: [{
                            ...
                            children: [{
								...
								/**Patch For Cell Spacing**/
								getValue: q,
                                validate: CKEDITOR.dialog.validate.cssLength(a.lang.common.invalidCssLength.replace("%1", a.lang.table.invalidCellSpacing)),
								onChange: function() {
                                        var a = this.getDialog().getContentElement("advanced", "advStyles");
                                        a && a.updateStyle("border-spacing", this.getValue());
										a.updateStyle("border-collapse", "separate");
                                    },
                                setup: function(a) {
                                        this.setValue(a.getStyle("border-spacing"))
                                    },
                                commit: k
								/**Patch For Cell Spacing**/
                            }, {
                                ...
                                commit: function(a, d) {
									...
									/**Patch For Cell Padding**/
									for (var i = 0; i < d.$.rows.length; i++) {
									var m = d.$.rows[i];
										for (var j = 0; j < m.cells.length; j++) {
											var b = d.$.rows[i].cells[j]
											this.getValue() ? b.setAttribute('style',"padding:" + this.getValue() + "px") : b.removeAttribute("style");;
										}
                                    }
									/**Patch For Cell Padding**/
                                }
