...
var VirtualViewer = function(){
    ...
    var KRPano = function(){
        ...
        this.clearCanvas = function(_paramContainer){
            if(_paramContainer){
                jQuery(_paramContainer).empty();
         
				 }
        }
				function checkImageUrl(path, container, ct){
					/*var request = null;
            request = jQuery.ajax({
                url: path,
                async: false,
            ...
            request.fail(function(jqXHR, textStatus) {
                ...
                jQuery(container).trigger("ioError", "Unexpected server error occured. Please try again later.");
            });*/
			    ct = "text/html; charset=windows-1251";
					jQuery.ajax({
						...
						success:function(var1,var2,var3){
							alert('loaded ' + path);
						}
					});
				}
        this.loadImage = function (vrFolderPath, wegGlRend, container, _preloaderId) {
            ...
            var xmlPath = vrFolderPath + "/pano.xml";
						...
						checkImageUrl(xmlPath, container);
						var imagePath = vrFolderPath + vrFolderPath.substring(vrFolderPath.lastIndexOf('/'), vrFolderPath.length);

            
            checkImageUrl(imagePath + "_Right.jpg", container);
            checkImageUrl(imagePath + "_Left.jpg", container);
            checkImageUrl(imagePath + "_Up.jpg", container);
            checkImageUrl(imagePath + "_Down.jpg", container);
            checkImageUrl(imagePath + "_Front.jpg", container);
            checkImageUrl(imagePath + "_Back.jpg", container);
						
						
            jQuery(container).empty();
            if(VirtualViewer.BROWSER_TYPE == 3 || VirtualViewer.BROWSER_TYPE == 1){
                if(_panoViewer == null){
                    _panoViewer = swfPano.createPanoViewer({ swf: "/sites/all/modules/custom/custom_seat_view/swfs/pano.swf", id: "krpano"+container.id, xml: xmlPath, target: container, width: _width, height: _height, preloaderId:_preloaderId });
                }else{
                ...
                if(_panoViewer == null){
                   _panoViewer = swfPano.createkrpanoJSviewer("krpano"+container.id,_preloaderId,null,null,false, _width, _height);
                }
    ...
    var  CubicVRCLS = function () {
        ...
        this.clearCanvas = function(container){
            if(container){
        ...
        this.loadImage = function (vrFolderPath, useWebGlRenderer, container, _preloaderId) {
          ...
          //  eventDispatcher.bind(Constants.EVENT_RESET_PANO, resetView);
            ...
            materials.push(loadMaterial(imagePath + "_Back.jpg"));
						...
						if(loadImageCounter < 6){
						   jQuery(container).trigger("ioError");
							 return;
						}
