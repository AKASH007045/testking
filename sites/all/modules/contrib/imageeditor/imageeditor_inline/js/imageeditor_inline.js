    ...
    addInlineEditor: function() {
      if (!$(this).hasClass('imageeditor-inline-processed')) {
        ...
        if (this.naturalWidth >= Drupal.settings.imageeditor_inline.min_dimention || this.naturalHeight >= Drupal.settings.imageeditor_inline.min_dimention) {
          ...
          $(this).wrap("<div class='imageeditor-inline-wrapper'></div>");
          ...
          if (Drupal.settings.imageeditor_inline.icons_position == 1) {
            var $imageeditor_inline_wrapper_div = $(this).parents('div.imageeditor-inline-wrapper');
            var $imageeditor_div = $imageeditor_inline_wrapper_div.find('div.imageeditor');
            // Wrapper should have relative position and the same float and display property as the image.
            $imageeditor_inline_wrapper_div.css({position: 'relative', float: $(this).css('float'), display: $(this).css('display')});
