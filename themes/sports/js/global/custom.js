...
jQuery(document).ready(function(){  
  ...
  jQuery('#gallery-slider a, .event-slider-box .tilehead a').click(function() {
    ...
    if(isNaN(jQuery(this).attr('id')) == false) {
	 ...
	 jQuery('.event-gallary-big-box .carousel').carousel({
           pause: true,
          interval: false
       });
  ...
  /**BottomBar Countdown Timer Days to two digits if 0 is present on first digit**/
  if(jQuery('.bottomBar .nextgame-timer .countdownHolder .countDays').length > 0){
	  var days_position = jQuery('.bottomBar .nextgame-timer .countdownHolder .countDays .position.digit2 .digit');
...
jQuery(document).ready(function(){
    ...
    /* Footer disable*/
    if(Drupal.settings.custom_hooks.ts){
	var ts = Drupal.settings.custom_hooks.ts;
	var days = Drupal.settings.custom_hooks.days;
	jQuery('#countdown_num_days').html(days);
	//console.log(days);
	jQuery('#homecountdown').countdown({
	  timestamp	: ts,
	...
	/**PAWP Countdown Timer Days to two digits if 0 is present on first digit**/
	  if(jQuery('#homecountdown .countDays,.home-page-countdown .countDays').length > 0){
		  var days_position = jQuery(this).find('.position.digit2 .digit');
		  var days_position_digit = days_position.html();
		  days_position_digit = Number(days_position_digit);
		  if(days_position_digit === 0){
				days_position.parent(".position").css("display","none");
	  ...
	  /**PAWP Countdown Timer Days to two digits if 0 is present on first digit**/
	  /**Remove quickedit on countdown timer on homepage**/
	  jQuery(".home-page-countdown .field-name-field-countdown-timer").hover( function(){
		jQuery("#quickedit-entity-toolbar").hide();
	    },
		function(){
		  jQuery("#quickedit-entity-toolbar").show();
		});
	  jQuery(".home-page-countdown .field-name-field-countdown-timer").removeClass("quickedit-processed quickedit-field quickedit-candidate quickedit-editable quickedit-highlighted");
	  jQuery(".home-page-countdown .field-name-field-countdown-timer").removeAttr("data-quickedit-field-id");
	  /**Remove quickedit on countdown timer on homepage**/
});